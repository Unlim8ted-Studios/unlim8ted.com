...
function removeBookReaderMode() {
  hero.classList.remove("book-reader-hero");
}

function getBookFileName(fileUrl) {
  try {
    const url = new URL(fileUrl, window.location.href);
    const pathName = url.pathname.split("/").pop() || "Book";
...
function createBookReaderButton(label, ariaLabel) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = "bookReader-control";
  button.textContent = label;
  button.setAttribute("aria-label", ariaLabel);
  return button;
}

async function renderBookReader(item) {
  hero.innerHTML = "";
  thumbs.innerHTML = "";
  hero.classList.add("book-reader-hero");

  const fileUrl = hasContent(item.file) ? item.file.trim() : "";

  const reader = document.createElement("section");
  reader.className = "bookReader";
  reader.setAttribute("aria-label", `${item.name || "Book"} reader`);

  const toolbar = document.createElement("div");
  toolbar.className = "bookReader-toolbar";

  const title = document.createElement("div");
  title.className = "bookReader-title";
  title.textContent = item.name || getBookFileName(fileUrl);

  const actions = document.createElement("div");
  actions.className = "bookReader-actions";

  const decreaseFontButton = createBookReaderButton(
    "A−",
    "Decrease text size"
  );

  const increaseFontButton = createBookReaderButton(
    "A+",
    "Increase text size"
  );

  const topButton = createBookReaderButton(
    "Top",
    "Scroll to the beginning"
  );

  const downloadLink = document.createElement("a");
  downloadLink.className = "bookReader-control";
  downloadLink.textContent = "Open file";
  downloadLink.href = fileUrl || "#";
  downloadLink.target = "_blank";
  downloadLink.rel = "noopener noreferrer";

  const content = document.createElement("div");
  content.className = "bookReader-content";
  content.tabIndex = 0;

  const loading = document.createElement("div");
  loading.className = "bookReader-loading";
  ...
  hero.appendChild(reader);

  if (!fileUrl) {
    loading.className = "bookReader-error";
    loading.textContent = "This book does not have a readable file.";
  ...
  function setReaderFontSize(nextSize) {
    fontSize = clamp(nextSize, 13, 32);

    const textElement = content.querySelector(".bookReader-text");
    if (textElement) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Book request failed with status ${response.status}`
      );
    }

    const text = await response.text();

    const bookText = document.createElement("article");
    bookText.className = "bookReader-text";
    bookText.textContent = text.trim() || "This book file is empty.";
    bookText.style.fontSize = `${fontSize}px`;

    content.replaceChildren(bookText);
  } catch (error) {
    console.error("Book reader error:", error);

    const errorMessage = document.createElement("div");
    errorMessage.className = "bookReader-error";

    const message = document.createElement("div");
    message.append(
      document.createTextNode(
        "The book could not be loaded inside the page. "
      )
    );

    const fallbackLink = document.createElement("a");
    fallbackLink.className = "bookReader-download";
    fallbackLink.href = fileUrl;
    fallbackLink.target = "_blank";
    fallbackLink.rel = "noopener noreferrer";
    fallbackLink.textContent = "Open the book file";

    message.appendChild(fallbackLink);
    message.appendChild(
      document.createTextNode(" in a new tab.")
    );
...
function normalizeItemForPage(raw) {
  ...
  item.file = item.file ?? item.file_url ?? item.download ?? "";
...
async function renderCurrentProduct(force = false) {
  ...
  if (isBook) {
    ...
    await renderBookReader(item);

    /*
     * A free book can be read directly, so "Buy Now" is changed
     * into an "Open Book" action.
     */
    if (Number(item.price || 0) <= 0 && hasContent(item.file)) {
      buyBtn.textContent = "Open Book";
      buyBtn.href = item.file;
    ...
    /*
     * Hide Add to Cart for free books. Change this condition if
     * free books should still be added to the user's library.
     */
    if (Number(item.price || 0) <= 0) {
      ...
      addBtn.onclick = async () => {
        try {
          ...
          speakAsMeatball("cart-add", {
            priceText: toMoney(Number(item.price || 0)),
            immediate: true
          }).catch(console.error);
        } catch (error) {
          console.error(
            "addToCartMerged book error:",
            error?.code,
            error?.message,
            error
          );
    ...
    (item.additional_videos || []).forEach((src) => {
      sources.push({
        type: "video",
        src
      });
